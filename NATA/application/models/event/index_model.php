...
class Index_model extends CI_Model
{
	...
	*/
	...
	function add_user()
	{
		...
		$data['address1'] = $this->session->userdata('add1');
                $data['address2'] = $this->session->userdata('add2');
                $data['state'] = $this->session->userdata('state');
                $data['zip'] = $this->session->userdata('zip');
                $data['country'] = $this->session->userdata('country');
                $data['homephone'] = $this->session->userdata('homephone');
                $data['filmtitle'] = $this->session->userdata('filmtitle');
                $data['filmname'] = $this->session->userdata('filmname');
                $data['filmduration'] = $this->session->userdata('filmduration');
                $data['filmgenre'] = $this->session->userdata('filmgenre');
                $data['addlink1'] = $this->session->userdata('addlink1');
                $data['addlink2'] = $this->session->userdata('addlink2');
