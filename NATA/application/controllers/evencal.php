...
class Evencal extends CI_Controller {
	...
	// for convert (int) month to (string) month in India
	...
	function do_add(){
		...
		if ($this->form_validation->run() == FALSE){
			...
			$this->evencal->addEvent($this->input->post('year'), 
											 ...
											 $this->input->post('day'), 
											 "09".":"."09".":00",
											 $this->input->post('event'),$this->input->post('title'));
			echo json_encode(array('status' => true, 'time' => $this->input->post('time'), 'event' => $this->input->post('event'), 'event_title' => $this->input->post('title')));
		}
	}
