...
class Calender extends CI_Controller {
    ...
    public function index($year = null, $month = null, $day = null) { /*     * script for calender data passing starts including index parameters
      $year,$month,$day(these only for calender events logic part only) */
        ...
        $data['title'] = 'Nata event calendar| North America Telugu Association monthly events';
        $data['keywords'] = 'Nata events calendar, monthly events in nata, nata December events, nata January events';
        $data['description'] = '';
    ...
    function _setting() {
        return array(
            ...
            'month_type' => 'long',
            'day_type' => 'short',
            'template' => '{table_open}<table class="date-big">{/table_open}
								   {heading_row_start}&nbsp;{/heading_row_start}
								   {heading_previous_cell}<caption><a href="{previous_url}" class="prev_date fa fa-chevron-left" title="Previous Month"></a><span class="current-month">{/heading_previous_cell}
								   ...
								   {cal_cell_content}<div class="date_event detail" val="{day}"><span class="date-big">{day}</span><span class="event event1 d{day}">{content}</span></div>{/cal_cell_content}
								   {cal_cell_content_today}<div class="active_date_event detail" val="{day}"><span class="date-big">{day}</span><span class="event event1 d{day}">{content}</span></div>{/cal_cell_content_today}
								   {cal_cell_no_content}<div class="no_event detail" val="{day}"><span class="date-big">{day}</span><span class="event event1 d{day}">&nbsp;</span></div>{/cal_cell_no_content}
								   {cal_cell_no_content_today}<div class="active_no_event detail" val="{day}"><span class="date-big">{day}</span><span class="event event1 d{day}">&nbsp;</span></div>{/cal_cell_no_content_today}
