...
class User_model extends CI_Model {
    ...
    function get_password() {
    	$email =$_POST['email'];
       $str="select email,password from user where email='$email' limit 1";
...
//$to1=date ('Y-m-d', strtotime($to));
	...
	$data=array(
            ...
            'lastname'=>$this->session->userdata('txtlname'),
           ...
           $this->db->insert('user',$data);
	}		public function add_member()     //add member access login	{	$data=array(			'username'=>$this->input->post('user_name'),			'email'=>$this->input->post('email_address'),			'password'=>$this->input->post('password'),			'pw_decode'=>$this->input->post('con_password'),			'member_name'=>$this->input->post('user_name'),			'user_level'=>$this->input->post('user_level'),			);           $this->db->insert('user',$data);	}
	public function add_doner()
	{
	$randam=rand(1000,10000);
	$data=array(
			'username'=>$this->session->userdata('d_email'),
			'email'=>$this->session->userdata('d_email'),
			'password'=>$randam,
			'member_name'=>$this->session->userdata('d_fname'),
            'lastname'=>$this->session->userdata('d_lname'),
			'dated'=>date('d-m-Y'),
			'address'=>$this->session->userdata('d_street'),
			'city'=>$this->session->userdata('d_city'),
			'state'=>$this->session->userdata('d_state'),
			'zip'=>$this->session->userdata('d_zip'),
			'cellphone'=>$this->session->userdata('d_phone'),
			'user_level'=>0,
			'memo'=>$this->session->userdata('d_memo'),
	...
	return $query->result_array();
	}		function members_access()                      //member access list	{	$this->db->order_by('id','desc');	$query=$this->db->get_where('user',array('user_level' => 5));	return $query->result_array();	}
